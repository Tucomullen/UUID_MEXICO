...
FORM frm_salvar_uuid
  ...
  IF sy-subrc = 0.
    ...
    IF lv_ok <> gc_stat_same.
      pv_error = 'X'.
    ELSE.
*     Registrar tdname en el set anti-duplicidad (clave: uuid + doc)
      DATA: lv_tdname_written TYPE tdobname.
      CONCATENATE pv_uuid ':' pv_bukrs pv_belnr pv_gjahr
        INTO lv_tdname_written.
      CONDENSE lv_tdname_written NO-GAPS.
      INSERT lv_tdname_written INTO TABLE gt_uuid_escrito.
    ENDIF.
  ELSE.
    pv_error = 'X'.
  ENDIF.
